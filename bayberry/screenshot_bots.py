...
class ScreenShotBots:
    def __init__(self, dir_name, urls_filename):
    ...
    def get_save_directory(self):
        save_path = self.dir_name + "-" + self.get_uniq_dir_name()
    ...
    def exec_phantomjs(self, datas, save_path):
        ...
        for r in datas:
            ...
            if len(pname_tmp) > self.max_fn_str_cnt:
                ...
                pname = pname_tmp
            print(pid, " ", pname_tmp, " を処理しています。")
            pjswd.get(url)
            time.sleep(self.shortWait)
            self.fullpage_screenshot("phantomJS", pjswd, save_path  + pid + "_" + pname + ".png")
        self.close_wd(pjswd)
...
params.add_argument('arg1', help='input the Save directory prefix')
params.add_argument('arg2', help='input the URLs file name')
args = params.parse_args()

dir_name = args.arg1
urls_filename = args.arg2
app = ScreenShotBots(dir_name, urls_filename)
app.exec()